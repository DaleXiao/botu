// lib/jobcore.js — SPEC-433/T-733 v4 异步 job 核心 + SPEC-435/T-736 v5 共享执行体
// 自 functions/api/generate.js 字节级抽取（ICON_PROMPT md5 锚 a90f21f3b840d5ff21e41a93f1efe53e 一字不动；
// 唯一代码改动 = runGeneration/runJob 的 opts.signal 注入点，供 DO alarm 5min 硬超时 abort）。
// 消费方：functions/api/generate.js（Pages POST 入口，re-export 保持既有 import 不变）、
// functions/api/result.js（经 generate.js 间接）、worker/src/index.js（DO JobRunner alarm 执行体）、test/*.js。
// quota 恰好一次：仅生成成功扣费一次（counted 标记落 job 值）；失败/超时不扣；env.__fetch 可覆写 fetch 供 mock。

export const MODEL = 'qwen-image-3.0-pro';
// Gateway 指令（Dale 2026-09-22 19:42）：出图不直连 DashScope，统一走 api-llm gateway
//（icon-forge/ukiyo-e 同款出口，SPEC-160/SPEC-285 先例）。Gateway 内部透传 dashscope，
// 上游响应 schema 不变 → buildBody/extractImage 解析逻辑 0 修改。
export const UPSTREAM_PATH = '/v1/images/generations';
export const LLM_USECASE = 'icon-image'; // x-llm-usecase（SPEC-285 icon 生成场景，与 icon-forge 一致）
...
export async function runGeneration(env, v, opts = {}) {
  ...
  // SPEC-440/T-742: 两腿拆分 — gateway POST 腿优先 service binding API_LLM 内网直连
  // api-llm-worker，绕过 CF edge ~100s 墙根治 524；__fetch（测试 mock）优先级最高。
  // 图片下载腿（extractImage 返回的外部绝对 URL，如 OSS）永不走 binding——
  // 非 /v1/* path 在 api-llm-worker 会 404 → 100% 失败（lynx ev20535），仍用 doFetch。
  const gwFetch = env.__fetch || (env.API_LLM ? (url, init) => env.API_LLM.fetch(url, init) : fetch);
  // SPEC-435: opts.signal — DO alarm 5min 硬超时 AbortController 注入点
  ...
  // SPEC-435: abort 后即使底层/mock 未 reject 也短路（5min 硬超时双保险）
  if (signal && signal.aborted) {
    return { ok: false, status: 502, error: TIMEOUT_ERROR };
  }
  if (!upstreamRes.ok) {
...
// ── SPEC-435/T-736 v5: DO alarm 执行体支撑 ─────────────────────
// 5min 硬超时：DO alarm handler 用 AbortController 注入 signal；abort 后 error 文本固定（无上游细节）。
export const TIMEOUT_ERROR = 'generation timed out';
export const JOB_HARD_TIMEOUT_MS = 300_000; // spec §方案A：handler 内硬超时 5min
export const MAX_JOB_ATTEMPTS = 3; // alarm attempts 上限：≥3 → error 终态停止重试（spec §方案A 步骤2）

// 图片载荷走 KV img:<id>（stub.fetch 触发链/alarm 不携带大载荷）：TTL 1h，与 job 记录一致。
...
// 写异常 → false（POST 分支语义同 jobWrite：失败 → 500 job store unavailable）
...
// 读 missing/损坏/异常 → null（alarm handler 据此写 error 终态，不扣费）
