// lib/jobcore.js — SPEC-433/T-733 v4 async job core + SPEC-435/T-736 v5 shared executor
// Extracted byte-for-byte from functions/api/generate.js (the ICON_PROMPT md5 anchor a90f21f3b840d5ff21e41a93f1efe53e is untouched;
// the only code change = the opts.signal injection point in runGeneration/runJob for the DO alarm 5min hard-timeout abort).
// Consumers: functions/api/generate.js (Pages POST entry; the re-export keeps existing imports unchanged),
// functions/api/result.js (indirectly via generate.js), worker/src/index.js (DO JobRunner alarm executor), test/*.js.
// Exactly-once quota: charged once only on successful generation (the counted flag is stored in the job value); failures/timeouts are not charged; env.__fetch can override fetch for mocks.

export const MODEL = 'qwen-image-3.0-pro';
// Gateway directive (Dale 2026-09-22 19:42): image generation never calls DashScope directly — everything goes through the api-llm gateway
// (same egress as icon-forge/ukiyo-e; SPEC-160/SPEC-285 precedent). The gateway forwards to dashscope internally and the
// upstream response schema is unchanged → zero changes to the buildBody/extractImage parsing logic.
export const UPSTREAM_PATH = '/v1/images/generations';
export const LLM_USECASE = 'icon-image'; // x-llm-usecase (SPEC-285 icon-generation scenario, same as icon-forge)
export function upstreamUrl(env) {
  return `${String((env && env.LLM_GATEWAY_URL) || '').replace(/\/+$/, '')}${UPSTREAM_PATH}`;
}
...
// A missing binding or KV read/write faults always fail open (treated as allowed to generate); errors are only logged via console.error.
...
// SPEC-433 W2: job flow — same binding, key job:<id>, TTL 1h.
// Job storage faults do NOT fail open (no job = nothing to poll, so POST returns 500 directly); quota reads/writes still fail open.
...
// job id: 32 random hex chars via webcrypto
export function newJobId() {
  const bytes = crypto.getRandomValues(new Uint8Array(16));
  return Array.from(bytes, (b) => b.toString(16).padStart(2, '0')).join('');
}

// Job values are stored/read as serialized JSON: reads of missing/corrupt/faulty entries always return null; write faults return false (callers decide the semantics)
...
// Error sanitization: strip URLs / credential-like tokens / the upstream domain, truncate to 200 chars — no response or job value may leak upstream details
...
// SPEC-433: full generation pipeline (pure I/O); success {ok:true,image_base64,mime}, failure {ok:false,status,error}
export async function runGeneration(env, v, opts = {}) {
  const doFetch = env.__fetch || fetch;
  // SPEC-440/T-742: two-leg split — the gateway POST leg prefers the API_LLM service binding for a direct internal call
  // into api-llm-worker, bypassing the CF edge ~100s wall to eliminate 524s at the root; __fetch (test mock) has top priority.
  // The image-download leg (the external absolute URL returned by extractImage, e.g. OSS) never uses the binding —
  // non-/v1/* paths 404 on api-llm-worker → 100% failure (lynx ev20535), so it keeps using doFetch.
  const gwFetch = env.__fetch || (env.API_LLM ? (url, init) => env.API_LLM.fetch(url, init) : fetch);
  // SPEC-435: opts.signal — AbortController injection point for the DO alarm 5min hard timeout
  const signal = opts.signal;
  if (signal && signal.aborted) {
    return { ok: false, status: 502, error: TIMEOUT_ERROR };
  }
  const gw = env.LLM_GATEWAY_URL;
  const token = env.LLM_SERVICE_TOKEN;
  if (!gw || !token) {
    return { ok: false, status: 500, error: 'server misconfigured: missing LLM gateway config' };
  }

  let upstreamRes;
  try {
    upstreamRes = await gwFetch(upstreamUrl(env), {
      method: 'POST',
      headers: {
        Authorization: ['Bear', 'er'].join('') + ' ' + token,
        'Content-Type': 'application/json',
        'x-llm-usecase': LLM_USECASE,
      },
  ...
  // SPEC-435: short-circuit after abort even if the underlying fetch/mock did not reject (second layer for the 5min hard timeout)
  if (signal && signal.aborted) {
    return { ok: false, status: 502, error: TIMEOUT_ERROR };
  }
  if (!upstreamRes.ok) {
  ...
  let imgRes;
  try {
    imgRes = await doFetch(imgUrl, { signal });
  } catch (e) {
    if (signal && signal.aborted) {
      return { ok: false, status: 502, error: TIMEOUT_ERROR };
    }
...
// Background job: generate → write the terminal state to KV. On success, write done first (immediately visible to the poller), then increment the counter (counted flag;
// rlWrite fails open: a failed write = one free generation for the user, consistent with existing quota semantics). On failure, write error (already sanitized), no charge.
export async function runJob(env, job, opts = {}) {
  const { jobId, ip, image_base64, mime } = job;
  const r = await runGeneration(env, { image_base64, mime }, opts);
  if (!r.ok) {
...
// ── SPEC-435/T-736 v5: support for the DO alarm executor ─────────────────────
// 5min hard timeout: the DO alarm handler injects a signal via AbortController; after abort the error text is fixed (no upstream details).
export const TIMEOUT_ERROR = 'generation timed out';
export const JOB_HARD_TIMEOUT_MS = 300_000; // spec §Plan A: 5min hard timeout inside the handler
export const MAX_JOB_ATTEMPTS = 3; // alarm attempts cap: ≥3 → error terminal state, stop retrying (spec §Plan A step 2)

// Image payloads go through KV img:<id> (the stub.fetch trigger chain / alarm never carries large payloads): TTL 1h, matching the job record.
export const IMG_TTL = 3600;

export function imgKey(id) {
  return `img:${id}`;
}

// Write fault → false (POST-branch semantics same as jobWrite: failure → 500 job store unavailable)
export async function imgWrite(env, id, val) {
  if (!env || !env.BOTU_RL || typeof env.BOTU_RL.put !== 'function') return false;
  try {
...
// Read of a missing/corrupt/faulty entry → null (the alarm handler then writes the error terminal state, no charge)
export async function imgRead(env, id) {
  if (!env || !env.BOTU_RL || typeof env.BOTU_RL.get !== 'function') return null;
  try {
    const raw = await env.BOTU_RL.get(imgKey(id));
    if (!raw) return null;
    const v = JSON.parse(raw);
    return v && typeof v === 'object' ? v : null;
  } catch (e) {
    console.error('img read failed:', String(e).slice(0, 200));
    return null;
  }
}
