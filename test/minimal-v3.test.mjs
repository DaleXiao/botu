...
test('gallery 换图锚: icon-1/2/5 md5 == v3 终值，icon-1 压缩后 ≤ 500000B', () => {
  assert.equal(md5('gallery/icon-1.png'), 'fc40f351965e7210a50261b1d447d79e');
  assert.equal(md5('gallery/icon-2.png'), 'f676717f5fd466c2f58619b876650168');
  assert.equal(md5('gallery/icon-5.png'), '1c3fa8b1c07f4c7f3fc0965bf42c5acb');
  assert.ok(readFileSync(join(ROOT, 'gallery/icon-1.png')).length <= 500000, 'icon-1 size gate');
});

test('gallery 不动锚: icon-3/4/6 + char-1..6 md5 == 原值', () => {
  const ORIG = {
    ...
    'gallery/char-2.png': '5c107520f1c10ce9b3ffc138720595d9',
    'gallery/char-3.png': '43f483a294c50591cc41ff5ff1a986c3',
    'gallery/char-4.png': 'a9b0a508e9d9057a7514f25d34d6346d',
    'gallery/char-5.png': '8e2ee5453cda99a58bd21f5982008698',
